...
const removeDate = (id) => {
    if (! window.confirm('Remove this availability period?')) return;

    // A dedicated POST endpoint works on shared/cPanel hosts that block or
    // mishandle browser DELETE requests and method spoofing.
    router.post(`/admin/availability/dates/${id}/remove`, {}, { preserveScroll: true });
};
